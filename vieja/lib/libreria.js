...
{ if (lin == 0)
   {locationSelect.innerHTML = "";
    var option = document.createElement("option");
       option.value = 0;
       option.innerHTML = "Ver todas las opciones:";
	...
	cart = '';
	document.getElementById('HorariosParadas').value=cart;
...
{	counter = 0;
	...
	locationSelect = document.getElementById("Seleccionar");  
...
function BuscarLocaciones()
  {   
	...
	var radio = document.getElementById('cantCuad').value; 
             ...
             if (status2 == google.maps.GeocoderStatus.OK) 
		        { 
				   ...
				   marcaDest = new google.maps.Marker({
					...
					var ori = origen[0].geometry.location;
					var dest = destino[0].geometry.location;
					
					PasanCerca(ori,dest,radio,0);
					}//if status2
					});//geocode destino
				}//if status		   
	});//geocode origen
 ...
 function MostrarOpciones(inic,fin,dist)
 {
 
  locationSelect.style.visibility = "visible";
	
  locationSelect.onchange = function() 
	{  var lin = locationSelect.options[locationSelect.selectedIndex].value;
	   var variables = lin.split ("-");
       var tipo = variables[0];
 
       if (tipo == 1 )
        { var l = variables[1];
	      BorrarSuperposiciones();
          PasanCerca(inic,fin,dist,l);
          }
       else 
		{
	      BorrarSuperposiciones();
	      var lin1 = variables[1];
	      var lin2 = variables[2];  
	      var radio = dist/10000;
	      BuscarCombinaciones(inic,fin,radio,lin1,lin2);
	      }
	 };
...
var UrlOrigen = 'xml/xmlPasanLineas.php?latI=' + desde.lat() + '&lngI=' + desde.lng() + '&latD=' + latPlaz + '&lngD=' + lngPlaz + '&radius=' + dist + '&l=' + lor; // + '&tip=' + 2;
var UrlDestino = 'xml/xmlPasanLineas.php?latI=' + latPlaz + '&lngI=' + lngPlaz + '&latD=' + hasta.lat() + '&lngD=' + hasta.lng() + '&radius=' + dist + '&l=' + ldet;// + '&tip=' + 2;
		 ...
		 downloadUrl(UrlDestino, function(dato)
		  {
			 ...
			 {	
				 ...
				 for (j=0; j<destino.length ;j++)
				 {
					 ...
					 if (lor==0) { var nro = nro+1;
					             crearOpcionCombi(lOrig,lDest,nro);
								 }

      				 GenerarHorarios(lOrig,parO,sent1);
	  ...
	  {  var xml = parseXml(data);
		 ...
		 for (v=0;v<pasa.length ;v++ )
		  {var linea = pasa[v].getAttribute("Linea");
		   ...
		   if (l==0) {crearOpcion(linea,nro);}
		
		   GenerarHorarios(linea,parO,sent);

		  } //for
	  
	  }); //download (searchUrl)
	
     MostrarOpciones(desde,hasta,distancia);		
	      
  }  //PasanCerca()

function crearOpcion(linea,nro)
{
if (nro!=0){ var option = document.createElement("option");
                  option.value ='1-'+linea;
                  option.innerHTML = nro +") "+linea;
                  locationSelect.appendChild(option);
				  }
		   else { var option = document.createElement("option");
		          option.value = '';
                  option.innerHTML = "No existe linea directa que pase por ambos puntos";
                  locationSelect.appendChild(option);
				  }	 
}//crearOpcion


function crearOpcionCombi(linea1,linea2,nro)
{
if (nro!=0){ var option = document.createElement("option");
                  option.value ='2-'+linea1+'-'+linea2;
                  option.innerHTML = nro +") "+linea1+ ' y luego la '+linea2;
                  locationSelect.appendChild(option);
				  }
		   else { var option = document.createElement("option");
		          option.value = '';
...
function GenerarHorarios(linea,pO,sent)
{	
	...
	downloadUrl(searchUrl, function(datas)
	  {
		...
		Generarcartel(linea,sent,t);
		
	  });  //downloadUrl(searchUrl)

}//GenerarHorarios()

function Generarcartel(Linea,Sent,tiempo)
  {
 ...
 downloadUrl(sUrls, function(data2) 
   {
	  ...
	  var arreglohoras=xml2.documentElement.getElementsByTagName("horario");
	   ...
	   if (arreglohoras.length==0){ cartel=cartel+"Finalizo con los recorridos por el dia de hoy. \n";
									 cartel=cartel+"----------------------\n";	
	                               }
	  else							
	   { 
	   ...
	   for (x=0;x<arreglohoras.length;x++)
		    { 
			 ...
			 cartel=cartel+ opcion + ")"+ NuevaHora +":"+ NuevoMinuto +" (en "+ DifMin + " min) aprox.\n";
...
{
	//map.clearOverlays(); //var puntosV=[]; var LIda,LVuelt;
  if (Sent=="ida")
     {var color = "#AD51E6"}
